...
public class JwtServiceImpl implements JwtService {
  ...
  /**
   * Lấy tất cả claims và đồng thời verify chữ ký bằng key tương ứng. Sử dụng
   * parserBuilder().setSigningKey(...).build().parseClaimsJws(token).getBody() — đây là cách phổ
   * biến để parse JWS claims (trường hợp token là signed JWS).
   */
  private Claims extractAllClaims(String token, TokenType tokenType) {
    ...
    return Jwts.parser()
        .verifyWith(key)
        .build()
        .parseSignedClaims(token) // tự kiểm tra token có bị sửa không,exp , nbf, iat (nếu có)
        .getPayload();
  }
