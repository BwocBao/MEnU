...
import com.example.MEnU.entity.PasswordResetToken;
...
import com.example.MEnU.exception.*;
import com.example.MEnU.repository.PasswordResetTokenRepository;
...
import com.example.MEnU.utils.DateUtil;
...
public class AuthServiceImpl implements AuthService {
  ...
  // (bằng repository hoặc EntityManager) đều nằm trong Persistence Context — còn gọi là Hibernate
  // session.
  ...
  public RegisterResponse register(RegisterRequest registerRequest) {
    ...
    mailService.sendMail(user.getEmail(), "Verify account", html, null);
  ...
  public LoginResponse login(LoginRequest loginRequest) {
    ...
    Date accessExpiredDate =
        jwtService.extractClaim(accessToken, Claims::getExpiration, TokenType.ACCESS);
    Date refreshExpiredDate =
        jwtService.extractClaim(refreshToken, Claims::getExpiration, TokenType.REFRESH);
  ...
  public RefreshTokenResponse refreshToken(String refreshToken) {
    ...
    if (daysLeft < 1) {
      ...
      Date refreshExpiredDate =
          jwtService.extractClaim(newRefreshToken, Claims::getExpiration, TokenType.REFRESH);
      LocalDateTime refreshExpiredLocal = DateUtil.dateToLocalDateTime(refreshExpiredDate);
      RefreshToken newTokenEntity =
          new RefreshToken(storedToken.getUser(), newRefreshToken, refreshExpiredLocal);
    ...
    Date accessExpiry =
        jwtService.extractClaim(newAccessToken, Claims::getExpiration, TokenType.ACCESS);

    LocalDateTime accessExpiryLocal = DateUtil.dateToLocalDateTime(accessExpiry);

    // 9. Trả response
    return new RefreshTokenResponse(newAccessToken, newRefreshToken, accessExpiryLocal);
  }
  ...
  public void sendResetPasswordLink(String email) {
    User user =
        userRepository
            .findByEmail(email)
            .orElseThrow(
                () -> new NotFoundException("Email ko tồn tại để gửi link reset password"));

    if (!user.isAccountNonLocked()) {
      throw new ForbiddenException("Account has been deleted");
    }
    // xóa hết token cũ rồi mới tạo token mới để tránh hacker chặn request lấy token các token chưa
    // sử dụng
    ...
    mailService.sendMail(user.getEmail(), "Reset your password", html, null);
  }
